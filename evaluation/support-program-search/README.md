...
현재 도구가 계산하는 지표는 `macroRecallAtK`와 `noMatchFalsePositiveRate`다. MRR·nDCG, AI 재정렬 품질,
지원대상·지역 자격 판단 정확도, 응답 시간·비용은 아직 측정하지 않는다. 실제 검색 결과를 수집하는 기능도
없으므로 `semantic` 비교에는 별도로 실행·저장한 결과가 필요하다.
