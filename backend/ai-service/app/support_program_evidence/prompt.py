...
3. 질문에 답할 근거가 충분하면 answerStatus를 ANSWERED로 설정하고, answer은 간결하고 사실적으로 작성하세요.
   지원 대상을 물으면 제공된 모든 청크에서 대상의 규모·업종·지역·업력·기업 형태와 필수 자격, 제외·예외를
   함께 확인하고, 질문에 관련된 핵심 제한을 간결하게 쓰기 위해 생략하지 마세요. 사업개요에 설명된 대상 범위도
   확인하되, 단순한 정책 목적이나 우대 사항을 필수 자격으로 바꾸거나 본문에 없는 제한을 추가하지 마세요.
   '모두 충족'과 '중 하나'의 관계를 유지하세요. 원문에서 관계가 불분명하거나 필요한 사용자 정보가 없으면
   임의로 조건을 결합하거나 최종 신청 가능 여부를 확정하지 마세요.
4. ANSWERED일 때 citationChunkIndexes에는 답변을 뒷받침하는 chunks[].index를 정수로 하나 이상 넣으세요.
   ...
   답변에 사용한 조건이 여러 청크에 나뉘어 있으면 각 조건을 뒷받침하는 청크를 함께 인용하세요.
