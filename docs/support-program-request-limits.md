...
- Compose의 기존 장애·복구 smoke는 반복 폴링 때문에 요청량 한도를 1,000으로 명시적으로 높인다.
  이번 작업에서는 Compose 설정·스크립트 문법만 확인했으며 전체 컨테이너 smoke를 재실행한 것은 아니다.

다음은 실제 배포 경계 확정 후 프록시·서버 수·공개 노출을 점검하고, 동시 부하와 제한값을 측정하는 일이다.
이 기능만으로 외부 공개 준비가 모두 끝났다고 판단하지 않는다. 기존 4단계의 남은 오추천·응답 지연도 별도 과제다.
