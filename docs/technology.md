...
모델 실행과 임베딩에 연결하고, Qdrant Client를 Index Service에, Recommendation Agent를 Ranking Service에
생성자로 전달합니다. 이 객체들은 `ApplicationContainer`에 보관되고 앱 종료 시 Client 연결을 닫습니다.
...
현재 업무 Agent는 점수화용 하나입니다. 임베딩과 벡터 검색을 별도 Agent로 구성하지 않았으며,
tool·handoff·graph 없이 `max_turns=1`로 점수화합니다. 실행 설정은
