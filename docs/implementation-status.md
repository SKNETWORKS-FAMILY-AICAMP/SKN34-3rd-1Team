...
| 스키마 관리 | 구현됨 | Flyway V1 공고, V2 동기화 세대, V3 공고별 공식 원문 테이블 |
...
| 실데이터 fixture 초안 내보내기 | `evaluation-fixture-export` profile이 MySQL의 현재 모든 제공처 `OPEN` 공고를 운영 색인 Mapper와 같은 ID·내용 해시·검색 문서로 JSON 기록 | 웹·동기화·Qdrant·AI·OpenAI 호출 없음; `cases: []`은 사람이 라벨링해야 함 |
