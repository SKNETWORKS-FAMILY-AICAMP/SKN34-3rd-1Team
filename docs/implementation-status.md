...
| 실데이터 검색 품질 | 실제 검색 캡처·실제 모델 비교 점수 없음 | 현재 제외 규칙상 질문 8개가 평가 가능하고 관련 공고가 있는 질문은 4개. 전체 16개 중 절반이 제외되며 최소 평가 완료 기준 미충족 |
...
| 우선순위 | 작업 | 완료를 판단할 기준 |
|---|---|---|
| 1 | 실데이터 검색 품질 평가 | 추가 판정은 한 번으로 종료. 남은 18건은 미확정으로 유지하고 동일 DB·색인/실행 비용을 확인한 뒤 실제 capture를 별도로 실행. 새 검색 후보를 풀에 병합·추가 판정한 후 후보 20개와 최종 Top-5를 비교. AI-only 출처·제외 질문을 명시 |
