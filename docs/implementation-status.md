...
다음은 5단계의 이미 구현된 기업마당 HTML 기반 RAG 답변·인용·근거 부족 처리의 검증이며, 새 RAG 구현은 아닙니다.
...
| 우선순위 | 작업 | 완료를 판단할 기준 |
|---|---|---|
| 1 | 5단계: 기존 원문 근거 답변의 품질·완료 기준 확인 | 기업마당 HTML 기반 RAG는 이미 구현됨. 고정 공고 질문으로 답변·원문 인용 일치와 근거 부족 응답을 검증하고 판정 출처·한계를 기록. PDF·첨부 확장은 별도 결정 |
