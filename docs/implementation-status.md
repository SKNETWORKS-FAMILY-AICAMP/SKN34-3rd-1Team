...
공식 사례는 기대 상태 6건 일치·AI 의미 검토 5건 일치/1건 부분 일치입니다. H01의 대상 범위 요약 누락과
...
| AI Service | Agent 출력·점수·자격 필터·일반·원문 근거 벡터 API·Compose 스텁 계약 테스트. 2026-09-07 로컬 전체 206개 통과 | 내부 계약과 검색·근거 청크 색인 동작 확인. 실제 Qdrant 연동 테스트는 CI 설정으로 별도 실행 |
...
| 우선순위 | 작업 | 완료를 판단할 기준 |
|---|---|---|
| 1 | 다음 제품 범위 선택 | 5단계 HTML 최소 범위 완료. 6단계 제공처/사용자 기능 중 하나를 선택하거나, 실제 서류 답변이 우선이면 첨부/PDF 수집·파싱·평가 범위를 별도로 결정 |
| 2 | 외부 공개 전 운영 검증 | 배포 경계와 신뢰할 프록시·서버 수를 확정하고 실제 동시 부하·제한값·상시 지연/실패/사용량 관측을 검증. 현재 요청 제한과 평가 실행 로그를 운영 검증 완료로 취급하지 않음 |
| 3 | 추가 검색·답변 품질 개선 | 후보 누락·오추천·지연과 RAG 대상 조건 요약 누락(H01)을 새 dev 근거와 같은 조건으로 전후 측정. 기존 평가 자료·미확정 질문을 보존하며 이미 노출된 heldout에 맞춰 튜닝하지 않음 |
