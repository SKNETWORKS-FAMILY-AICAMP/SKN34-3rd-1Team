...
기준일: 2026-09-06. 현재 저장소의 production 코드·설정·테스트에 있는 기능을 정리합니다.
...
**5단계는 진행 중**이며 기존 기업마당 HTML RAG 코드 검수·회귀 테스트 보강·고정 근거 답변 평가를 진행했습니다.
가상 공고로 실제 모델 12회 호출 중 유효 답변 11개를 얻었고 초기 검증 실패 1건·미실행 질문 1개가 남았습니다.
실제 공고의 전체 RAG 경로 품질 검증은 아직 완료하지 않았습니다.
[5단계 검수 범위와 실행법](../evaluation/support-program-evidence/README.md)을 참고하세요. 새 RAG 구현은 아닙니다.
...
| AI Service | Agent 출력·점수·자격 필터·일반·원문 근거 벡터 API·Compose 스텁 계약 테스트. 2026-09-06 로컬 전체 192개 통과 | 내부 계약과 검색·근거 청크 색인 동작 확인. 실제 Qdrant 연동 테스트는 CI 설정으로 별도 실행 |
...
| 5단계 RAG 코드 검수·부분 평가 | Core 테스트 19개 보강 후 전체 340개 통과. 가상 공고 실제 답변 12회 중 11개 유효, AI 의미 검토 일치 10개·부분 1개 | 초기 검증 실패 1건·E12 미실행 보존. 고정 답변 근거 평가이며 실제 원문 수집·Qdrant 검색 품질 평가는 아님 |
...
| 우선순위 | 작업 | 완료를 판단할 기준 |
|---|---|---|
| 1 | 5단계: 남은 사례·전체 근거 경로 평가 | 고정 근거 부분 평가 완료. E12 확인·초기 검증 실패 원인 추적·실제 공고의 수집→검색→답변·인용 확인이 남음. PDF·첨부 확장은 별도 결정 |
| 2 | 외부 공개 전 운영 검증 | 배포 경계와 신뢰할 프록시·서버 수를 확정하고 실제 동시 부하·제한값·상시 지연/실패/사용량 관측을 검증. 현재 요청 제한과 평가 실행 로그를 운영 검증 완료로 취급하지 않음 |
| 3 | 추가 검색 품질 개선 | 남은 후보 누락·오추천·지연을 새 dev 근거와 같은 조건의 전후 측정으로 개선. 기존 평가 자료와 미확정·제외 질문을 보존하며 이미 노출된 heldout에 맞춰 튜닝하지 않음 |
